...
The diagram distinguishes the implemented Stage 4B canonical pipeline and Stage 4C point-in-time engine (green) from future work (grey). The authoritative source was acquired and audited in Stage 2 and is not stored in Git. PostgreSQL remains the approved storage target, not an installed or integrated dependency.
...
flowchart LR
    ...
    M[Final feature materialisation<br/>planned Stage 4E]
    ...
    class B,V,L,D implemented;
    class C,M,E,F,G,H future;
    class A external;
```

## Component responsibilities

| Component | Responsibility | Status through Stage 4C |
|---|---|---|
...
- **Batch first, local first:** Stage 4B builds canonical local output; 4C implements point-in-time features; 4D will add PostgreSQL; 4E will complete feature materialisation and acceptance. This approved implementation sequence changes the original database-first ordering, not the canonical PostgreSQL schema or the eventual storage target. Streaming file iteration here is a memory technique, not an event-streaming service.
