...
Stage 4B canonical runs are also local-only, for example under `data/interim/canonical/stage4b/`. A completed run contains `canonical.csv` and `manifest.json`; a failed/interrupted run remains in a sibling `.incomplete-...` directory and must not be consumed as complete. The canonical CSV retains all transactions, labels, IDs and post-balances and is not a predictive feature dataset. Do not commit canonical CSVs or redistribute derived external data under the source-code MIT licence. See the [Stage 4B runbook](../docs/stage4b-canonical-pipeline.md).
